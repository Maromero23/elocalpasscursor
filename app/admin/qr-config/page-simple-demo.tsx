...
export default function QRConfigPage() {
  ...
  const saveButton = (buttonNumber: number) => {
  ...
  const isButtonSaved = (buttonNumber: number) => savedButtons.has(buttonNumber)

  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-8">QR Configuration System</h1>
            ...
            {[1, 2, 3, 4, 5].map(num => (
              <div 
                ...
                className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-lg ${
                  isButtonSaved(num) ? 'bg-green-500' : 'bg-gray-400'
                }`}
              >
          ...
          {[1, 2, 3, 4, 5].map(num => (
            <div key={num} className="bg-white rounded-lg shadow p-6">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold">Configuration {num}</h3>
                <button 
