...
`
    ...
    let emailSent = false
    try {
      // Use the shared email template system with proper debugging
      const { sendWelcomeEmailWithTemplates } = await import('@/lib/email-service')
      
      console.log(`📧 SELLER ROUTE: Using shared email template system for ${clientEmail}`)
      console.log(`📧 SELLER ROUTE: Seller savedConfigId: ${seller.savedConfigId}`)
