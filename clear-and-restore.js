...
async function clearAndRestore() {
  try {
    ...
    await prisma.user.deleteMany({ where: { role: 'LOCATION' } })
    await prisma.distributor.deleteMany({})
    await prisma.user.deleteMany({ where: { role: 'DISTRIBUTOR' } })
    ...
    // Update users with distributor IDs
    await prisma.user.update({
      where: { id: dist1User.id },
      data: { distributorId: distributor1.id }
    })

    await prisma.user.update({
      where: { id: dist2User.id },
      data: { distributorId: distributor2.id }
    })
    ...
    // Update location users with location IDs
    await prisma.user.update({
      where: { id: loc1User.id },
      data: { locationId: location1.id }
    })

    await prisma.user.update({
      where: { id: loc2User.id },
      data: { locationId: location2.id }
    })
